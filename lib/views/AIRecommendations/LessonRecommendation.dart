...
class LessonRecommendation extends StatefulWidget {
  const LessonRecommendation({Key? key}) : super(key: key);

  @override
  State<LessonRecommendation> createState() => _LessonRecommendationState();
}

class _LessonRecommendationState extends State<LessonRecommendation> {
  int? _userId;
  int selected = -1;
  int selected_2 = -1;
  List<Map<String, dynamic>> list = [];
  List lessonRecommendationData = [];
  ...
  initializeApi(String loaderMessage) {
    ...
    getUserDetail().then((res) async {
      ...
      _aIRecommendationService
          ...
          .then((lessonData) {
        setState(() {
          lessonRecommendationData = lessonData;
        });
        print("Test Recommendation Data : ${lessonRecommendationData}");
        closeLoader();
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: lessonRecommendationData.isEmpty
          ? Center(
              child: Text("No data"),
            )
          : ListView.builder(
              itemCount: lessonRecommendationData.length,
              itemBuilder: (context, index) {
                List topicName = [];
                Map data = lessonRecommendationData[index]['recommendation'];
                return Container(
                  width: double.infinity,
                  ...
                  child: ExpansionTile(
                      ...
                      onExpansionChanged: (val) {
                        if (val) {
                          ...
                          print(lessonRecommendationData[index]
                              ['recommendation']);
                          if (lessonRecommendationData[index]
                                  ['recommendation'] !=
                              null) {
                            data.forEach((k, v) {
                              topicName.add(k);
                      ...
                      title: Text(
                        lessonRecommendationData[index]['booking_data']
                                ['requested_date'] +
                            ' ' +
                            lessonRecommendationData[index]['booking_data']
                                ['requested_time'],
                      ...
                      children: lessonRecommendationData[index]
                                  ['recommendation'] ==
                              null
                          ? [
                              Center(
